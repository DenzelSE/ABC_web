        ...
        {Object.keys(forms).map((key) => (
          <button
            ...
            className={`px-6 py-3 font-semibold rounded-lg transition-all duration-300
              ${activeForm === key ? 'bg-[#AFAFAF] shadow-lg shadow-blue-500' : 'bg-[#D8CFC4] hover:bg-gray-600'}`}
          >
      ...
      <div className="bg-[#EFE6DC] p-10 rounded-2xl w-[600px]  mx-4 text-black text-center shadow-lg shadow-gray-900">
        ...
        <form className="space-y-4">
          {forms[activeForm].fields.map((field) => (
            field.type === 'select' ? (
              <select key={field.name} className="w-full p-3 bg-[#D8CFC4] rounded-lg focus:ring-2 focus:ring-blue-500">
